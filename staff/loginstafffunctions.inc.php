...
function LoginUser($conn, $email, $pwd){
    ...
    if ($userExists===false) {
        header("Location:../staff/stafflogin.php?error=wronglogin");
        exit();
    }
    ...
    if ($checkpwd===false) {
        ...
        header("Location: ../staff/staffoptions.php");
        exit();
    }
}

function emailExists($conn, $email) {
    $sql = "SELECT * FROM staff WHERE UsersEmail = ?;";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../staff/stafflogin.php?error=stmtfailed");
        exit();
    }
